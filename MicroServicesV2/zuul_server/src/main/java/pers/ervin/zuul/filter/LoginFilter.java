package pers.ervin.zuul.filter;
...
public class LoginFilter extends ZuulFilter {
  ...
  /** 当前过滤器是否生效 true : 使用此过滤器 flase : 不使用此过滤器 */
  public boolean shouldFilter() {
    return true;
  }

  /**
   * 指定过滤器中的业务逻辑 身份认证: 1.所有的请求需要携带一个参数 : access-token 2.获取request请求 3.通过request获取参数access-token
   * 4.判断token是否为空 4.1 token==null : 身份验证失败 4.2 token!=null : 执行后续操作
